...
similar to ASCII but not the same. It includes symbols that are
not in ASCII. To support these, we use 3-character sequences called
"trigraphs".

The simplest form of a trigraph is a backslash followed by three decimal
digits. This specifies a literal representation as a character. (For
example, "\\65" should encode as the a literal 0x41 character, which is the
letter "A".
